...
5. For authorized changes, pin direct versions, update the lockfile with pnpm, and keep changes scoped. Never use force, legacy-peer-deps, or ignored errors to hide incompatibility.
6. Verify pnpm install --frozen-lockfile, pnpm list --depth Infinity, pnpm audit, pnpm run check, and relevant production behavior. Explain advisories that remain; do not apply forced audit fixes.
